...
const ANALYZE_TIMEOUT_MS = Number(import.meta.env.VITE_ANALYZE_TIMEOUT_MS || 310000);
const ANALYZE_TIMEOUT_SECONDS = Math.round(ANALYZE_TIMEOUT_MS / 1000);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ fontSize: 12, color: C.textMuted, marginBottom: 32 }}>
        {elapsed < 40 ? "This can take 1-5 minutes after the model change" : `Still analyzing... (${elapsed}s)`}
      </div>
...
function ResultsScreen({ data, preferredLang = "en", onReset, onRecompute }) {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("PDF generation failed. If running locally, WeasyPrint requires GTK native libs. In production (Railway) this should work — check backend logs.");
      }
  ...
  const handleAnalyze = async (file, selectedLang) => {
    ...
    try {
      ...
      if (e.name === 'AbortError') {
        setError(`Request timed out after ${ANALYZE_TIMEOUT_SECONDS} seconds. The API did not respond in time. Please try a smaller file or check Railway backend logs for slow Gemini calls.`);
      } else if (e instanceof TypeError && /fetch/i.test(e.message || "")) {
        setError(`Could not reach the analysis API at ${API_URL}. Check that VITE_API_URL is set correctly in Vercel and that CORS_ORIGINS in Railway includes your Vercel domain.`);
      } else {
